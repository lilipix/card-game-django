...
def create_game(user) -> Game:
    ...
    if not user.is_authenticated:
        raise ValueError(
            "Un utilisateur authentifié est nécessaire pour créer une partie."
        )
...
def join_game(game: Game, user) -> GamePlayer:
    ...
    if not user.is_authenticated:
        raise ValueError(
            "Un utilisateur authentifié est nécessaire pour rejoindre une partie."
        )
    ...
    if locked_game.status != Game.Status.WAITING:
        raise ValueError(
            "Seule une partie en attente peut être rejointe."
        )
    ...
    if GamePlayer.objects.filter(game=locked_game, user=user,).exists():
        raise ValueError(
            "Cet utilisateur participe déjà à cette partie."
        )

    # Une partie de bataille doit déjà avoir un créateur avant d'être rejointe.
    players_count = GamePlayer.objects.filter(game=locked_game,).count()

    if players_count == 0:
        raise ValueError(
            "Cette partie ne possède pas de joueur 1."
        )
    ...
    if GamePlayer.objects.filter(game=locked_game,position=GamePlayer.Position.PLAYER_TWO,).exists():
      raise ValueError(
          "La position du joueur 2 est déjà occupée."
      )
...
def start_game(game: Game) -> Game:
      ...
      if locked_game.status != Game.Status.WAITING:
        raise ValueError(
            "Seule une partie en attente peut être démarrée."
        )

      # Récupère le joueur 1.
      player_one = GamePlayer.objects.filter(game=locked_game,position=GamePlayer.Position.PLAYER_ONE,
      ).first()

      # Récupère le joueur 2.
      player_two = GamePlayer.objects.filter(game=locked_game,position=GamePlayer.Position.PLAYER_TWO,
      ).first()

      # La partie ne peut démarrer que lorsque les deux joueurs sont présents.
      if player_one is None or player_two is None:
        raise ValueError(
            "Deux joueurs sont nécessaires pour démarrer la partie."
        )

      # Protection contre une éventuelle incohérence en base.
      players_count = GamePlayer.objects.filter(game=locked_game,
      ).count()

      if players_count != 2:
        raise ValueError(
            "La partie doit contenir exactement deux joueurs."
        )
      ...
      deck = [
        (suit, rank)
        for suit, _ in GameCard.Suit.choices
        for rank, _ in GameCard.Rank.choices
      ]
